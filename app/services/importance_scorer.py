...
def normalise_importances(importances: dict[str, float]) -> dict[str, float]:
	...
	normalized = {feature: float(value) / total for feature, value in importances.items()}
...
def rank_features_by_weighted_score(
	...
	weights = normalise_importances(importances) if importances else equal_weight_importances(feature_names)
	...
	for row in drift_scores:
		...
		weighted_score = compute_weighted_score(psi_normalised, weights.get(feature_name, 0.0))
