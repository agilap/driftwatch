...
async def run_weekly_reports_job() -> None:
	...
	async with AsyncSessionLocal() as db:
		result = await db.execute(select(ModelRegistry).order_by(ModelRegistry.name.asc()))
		...
		for model in models:
			...
			logger.info("Weekly report generated for %s: score=%s", model.name, report.overall_score)
